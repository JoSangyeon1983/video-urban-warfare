...
2026-06-19 | UPDATE | 시트 프롬프트 | §공통 권고 line25 결정 정정: 한 장 요소축소 → 컨셉별 전면 분리(잔재 모순 해소)
2026-06-19 | UPDATE | 시트 프롬프트 | 검수표 line20 장비·액션 분리 권고 (미채택)→(채택: 분리)로 정정 — line59 실제 분리와 일치
2026-06-19 | UPDATE | 시트 프롬프트·설정 한서진 중위 | 장비 명칭 통일: '제트팩+미사일 포드'/'백팩 멀티셀 포드 시스템' → '택티컬 백팩'(제트팩+멀티셀 포드 통합)
2026-06-19 | UPDATE | 시트 프롬프트 | 영문 프롬프트 장비 호칭을 tactical backpack으로 통일(캐릭터/장비/액션/히어로). missile-pod system→backpack pod system(콘텐츠 가드), 하위 부품 pod system·drone의 jetpack soldier는 유지
2026-06-19 | UPDATE | 시트 프롬프트·설정 한서진 중위 | 택티컬 백팩 한-장 생성 일관성 실패(접힘↔펼침 불가능·부스터 방향 불일치, 반려 이미지 아카이브) → 단계별 앵커 연쇄로 재설계: ①FOLDED 정본 단독→②참조 첨부 DEPLOYED 파생→③부스터 분리, 클램셸+격자 셀 축소+대칭. 설정에 클램셸 메커니즘 명문화
